...
export default function AcademicStats() {
  ...
  const questionCount = (() => {
    const paginatedQuestions = questionsData?.data as
      | {
          data?: unknown[]
          items?: unknown[]
          questions?: unknown[]
    ...
    return (
      paginatedQuestions?.data?.length ??
      paginatedQuestions?.items?.length ??
      paginatedQuestions?.questions?.length ??
      0
    )
  })()
  ...
  const stats = [
    ...
    {
      title: "Questions",
      value: questionsData?.data?.meta?.total ?? 0,
      icon: FileQuestion,
      color: "text-violet-600",
    },
  ]
  console.log(questionsData?.data?.meta?.total)
