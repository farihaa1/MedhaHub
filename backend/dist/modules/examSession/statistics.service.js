...
const generateStatistics = async (userId) => {
    const sessions = await examSession_model_1.ExamSession.find({
        userId,
        status: "submitted",
    });
    ...
    sessions.forEach((session) => {
        ...
        const sessionCorrect = session.answers.filter((answer) => answer.isCorrect).length;
        const sessionWrong = session.answers.filter((answer) => !answer.isCorrect).length;
